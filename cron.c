#include "types.h"
#include "user.h"
#include "date.h" //the rtcdate struct is defined here.
#include "fcntl.h"

#define WAIT 6000 //one minute
#define NULL ((void*) 0)
...
{
    int size;
    int run_when_matched[60]; // Make this dynamic later
} cronarray;
...
int main(int argc, char *argv[])
{
    ...
    date(&r); //pass rtcdate struct to date system call
    if (r.second > 0) //if this isn't the 0th second in the minute
    {
        sleep(100 * (60 - r.second)); //sleep until the end of the minute.
    }


    //Open cron file and read contents
    if ((file = open("crontab", O_RDWR)) < 0)
    {
        printf(1, "could not open cron file\n");
        exit();
    }

    read_file(file, contents); //I'm currently only checking the cron file at the beginning of each session, instead of checking between every run. This was due to a bug I ran out of time to fix.
    close(file);

    for(;;){
        
        date(&r); //grab the current time info every iteration
        //printf(1, "\n\nThe current time: %d:%d:%d. Day: %d. Month: %d\n\n", r.hour, r.minute, r.second, r.day, r.month);
        //printf(1, "this is iteration # %d\n", iterations); //testing for errors after letting it run a long time. Got to over a few hundred. Nothing failed.
        ...
        for (int i = 0; i < contents->count; i++)
        {
            ...
            //sleep(10); //sleeps for .1 second between commands so that echos appear nicely.
        }
        ...
        iterations++;
        sleep(WAIT); //this will check the if the commands match every minute
    }
